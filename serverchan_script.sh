...
if [ "$serverchan_notify_1" = "1" ] ; then
    ...
    if [ "$lastIP" != "$hostIP" ] && [ ! -z "$hostIP" ] ; then
        ...
        curl -L -s "http://sc.ftqq.com/$serverchan_sckey.send?text=【网络终端_"`nvram get computer_name`"】互联网IP变动：" -d "&desp=${hostIP}" &
...
if [ "$serverchan_notify_2" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复.txt" ] ; then
        ...
        curl -L -s "http://sc.ftqq.com/$serverchan_sckey.send?text=【网络终端_"`nvram get computer_name`"】新设备加入：" -d "&desp=${content}" &
...
if [ "$serverchan_notify_4" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复_上线.txt" ] ; then
        ...
        curl -L -s "http://sc.ftqq.com/$serverchan_sckey.send?text=【网络终端_"`nvram get computer_name`"】设备【上线】Online：" -d "&desp=${content}" &
    ...
    if [ -s "/tmp/var/newhostname不重复_下线.txt" ] ; then
        ...
        curl -L -s "http://sc.ftqq.com/$serverchan_sckey.send?text=【网络终端_"`nvram get computer_name`"】设备【下线】offline：" -d "&desp=${content}" &
...
if [ "$serverchan_notify_3" = "1" ] && [ "$resub" = "1" ] ; then
    ...
    if [ "$(cat /tmp/var/osub |head -n1)"x != "$(cat /tmp/var/nsub |head -n1)"x ] && [ -f /tmp/var/nsub ] ; then
        ...
        curl -L -s "http://sc.ftqq.com/$serverchan_sckey.send?text=【网络终端_"`nvram get computer_name`"】固件更新提醒：" -d "&desp=${content}" &
